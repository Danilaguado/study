...
const StudyTracker = () => {
  ...
  const handleFocusComplete = (minutosEstudiados) => {
    ...
    if (registroHoy) {
      const registrosActualizados = registrosCategoria.map((r) =>
        r.fecha === hoy ? { ...r, valor: r.valor + minutosEstudiados } : r
      );
      setRegistros({
        ...registros,
      ...
      const nuevosRegistrosCategoria = [
        ...registrosCategoria,
        nuevoRegistroFocus,
  ...
  const eliminarCategoria = (categoria) => {
    if (!window.confirm(`¿Eliminar "${categoria}" y todos sus registros?`))
      return;
  ...
  const agregarRegistro = () => {
    ...
    if (
      nuevoRegistro.realizado &&
      (!nuevoRegistro.valor || nuevoRegistro.valor <= 0)
    ) {
    ...
    const registrosCategoria = registros[categoriaActual] || [];
    const nuevosRegistrosCategoria = [...registrosCategoria, registro].sort(
  ...
  const datosGrafico =
    categoriaActual && registros[categoriaActual]
      ? registros[categoriaActual]
  ...
  const registrosActuales = categoriaActual
    ? registros[categoriaActual] || []
    : [];
  const registrosMostrar = mostrarMasRegistros
    ? registrosActuales
    : registrosActuales.slice(-10);
                ...
                <div className='bg-white rounded-lg shadow-lg p-6 mb-6'>
                  <div className='flex justify-between items-center mb-4'>
                    ...
                    <select
                      value={diasTendencia}
                      onChange={(e) =>
                        setDiasTendencia(parseInt(e.target.value))
                      }
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4'>
                    ...
                    <div className='bg-white rounded-lg p-4 border-2'>
                      ...
                      <p
                        className={`text-2xl font-bold ${getTendenciaColor(
                          metricas.tendencia
                        )}`}
                      >
                    ...
                    <div className='bg-green-50 rounded-lg p-4'>
                      ...
                      <p className='text-3xl font-bold text-green-600'>
                        {metricas.probabilidadCompletar}%
                      </p>
                    </div>
                  ...
                  <div className='bg-white rounded-lg shadow-lg p-6'>
                    ...
                    {registrosActuales.length > 10 && (
                      <div className='mt-4 text-center'>
                        <button
                          onClick={() =>
                            setMostrarMasRegistros(!mostrarMasRegistros)
                          }
                          className='text-indigo-600 hover:text-indigo-800 text-sm font-medium'
                        >
                          {mostrarMasRegistros
                            ? "Mostrar menos"
                            : `Ver ${registrosActuales.length - 10} más...`}
                        </button>
                      </div>
                    )}
                  </div>
